...
export default function App() {
  ...
  const lastMoodContextRef = useRef(lastMoodContext);
  ...
  useEffect(() => { lastMoodContextRef.current = lastMoodContext; }, [lastMoodContext]);
  ...
  useEffect(() => {
    const trackChangedListener = TrackPlayer.addEventListener(Event.PlaybackTrackChanged, async (event) => {
      ...
      if (track) {
        ...
        // Ensure playback continues on web/mobile if it paused during transition
  ...
  const fetchSimilarSongs = async (song, mood) => {
    try {
      ...
      const l = currentLanguageRef.current;
      let query = `${l} ${mood} songs`;

      // If we have a specific mood like melody, add romantic keywords
      if (mood === 'melody') query = `${l} love romantic songs`;
      else if (mood === 'item') query = `${l} dance party songs`;
      else if (mood === 'chill') query = `${l} chill slow songs`;
